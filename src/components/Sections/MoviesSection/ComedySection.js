import React,{ useEffect } from 'react'

//Redux
import { useSelector, useDispatch } from 'react-redux'
import { getComedyMovies } from '../../../redux/actions/movieActions'

//Components
import MovieCard from '../../MovieCard'

//Styled components
import { SectionCards, SectionContainer, SectionTitle } from './MoviesElements'

const ComedySection = ({ sectionTitle, isTrending }) => {
  const dispacth = useDispatch()
  useEffect(() =>{
    dispacth(getComedyMovies())
  },[dispacth])

  //state
  const comedyMovies = useSelector( state => state.comedyMovies )
  const { loading, error, comedies} = comedyMovies

  return (
    <SectionContainer>
      <SectionTitle>{sectionTitle}</SectionTitle>
      {
        loading ? (<h2>Loading....</h2>) : error ? (<h2>{error}</h2>) :(
          <SectionCards>
            { comedies.map(result => (
              <MovieCard isTrending={isTrending} key={result} result={result}/>
            ))}
          </SectionCards>
        )
      }
      
    </SectionContainer>
  )
}

export default ComedySection
